...
At which percentage the method captures the cell types significantly?
...
```{r sens_plot, fig.width=7, fig.height=6, echo=FALSE}
sensitivity_plot %>%
  mutate(min_frac=ifelse(is.infinite(min_frac), 100, min_frac)) %>% 
...
```{r, sens_spec_plot, fig.height=6, fig.width=6, echo=FALSE}
specificity_data %>% 
...
<!-- ### Separate test for MCP counter -->

<!-- //this does not make sense, as the null models are platform specific. --> 

<!-- Since MCPcounter score does not have a clear "0", as it uses raw gene expression values,  -->
<!-- it cannot be directly compared to the other methods in the figure above.  -->

<!-- However, it does come with a function `test_for_infiltration` which can be used to formally test -->
<!-- if an immune cell type is detected or not.  -->


<!-- ```{r} -->
<!-- map_celltype_to_mcp = cell_type_mapping %>% filter(method_dataset == "mcp_counter") -->
<!-- mcp_results = all_results %>% -->
<!--   filter(method == "mcp_counter", n_immune_cells == 0) %>% -->
<!--   mutate(sample_id = paste(input_cell_type, sample, sep="_")) %>%  -->
<!--   select(cell_type, sample_id, estimate) %>%  -->
<!--   spread(sample_id, estimate) %>%  -->
<!--   inner_join(map_celltype_to_mcp) %>%  -->
<!--   select(-cell_type, -method_dataset) %>%  -->
<!--   as.data.frame() %>%  -->
<!--   column_to_rownames("method_cell_type") -->

<!-- MCPcounter::test_for_infiltration(mcp_results) -->
<!-- ``` -->