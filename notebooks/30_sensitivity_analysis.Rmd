...
res = foreach(input_cell_type = show_cell_types,
  ...
  foreach(method = config$deconvolution_methods,
          .final = function(x) setNames(x, config$deconvolution_methods)) %dopar% {
            ...
            # TIMER and xCell predictions depend on the batch, not only on the samplese.
            # Therefore we run TIMER in five independent batches to get a better estimate of the
            # confidence interval
            n_batches = 5
            batch_length = length(unique(n_immune_cells))
            batch_res = lapply(1:n_batches, function(i) {
              ind = ((i-1)*batch_length + 1):(i*batch_length)
              deconvolute(expr_mat[,ind], method, indications=timer_indications[ind]) 
            })
            Reduce(full_join, batch_res) 
          }
