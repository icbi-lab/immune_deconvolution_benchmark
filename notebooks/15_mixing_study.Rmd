...
```{r, fig.width=8, fig.height=6, echo=FALSE, fig.cap="Correlations of predicted vs. known fractions on 100 simulated bulk RNA-seq samples, organized by cell type. "}
correlations %>%
...
```{r, fig.width=8, fig.height=6, echo=FALSE, fig.cap="Comparison of absolute methods. The bars indicate the log2(slope). Values < 0 indicate an under-prediction of the cell type, values > 0 an over-prediction respectively. "}
log_slope %>%
  right_join(cell_type_method_mat) %>%
  # na -> 0 workaround, otherwise text label does not show
  ggplot(aes(x=method, y=ifelse(is.na(log_slope), 0, log_slope), alpha=macrophage_only)) +
    geom_bar(aes(fill=method), stat="identity") +
    geom_text(aes(label=ifelse(is.na(log_slope), "n/a", NA)), angle=90) + 
...
```{r abs_rmse, fig.width=10, fig.height=6, echo=FALSE, fig.cap="Comparison of absolute methods. The values show the RMSE"}
...
rmse %>% 
 right_join(cell_type_method_mat) %>%
 ggplot(aes(x=method, y=ifelse(is.na(rmse), 0, rmse), alpha=macrophage_only)) +
    geom_bar(aes(fill=method), stat="identity") +
