...
plot_cor_table = function(data) {
  ...
  data %>%
    ...
    ggplot(aes(x=column, y=method_name)) +
      ...
      facet_wrap(~category, scales = "free", strip.position="right", nrow=1) +
...
validation$all_datasets = validation$all_results %>%
  ...
  mutate(dataset_group = "All datasets")

validation$all_datasets_all_cells = validation$all_results %>%
  group_by(method) %>%
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(cell_type = ALL_CELLS, dataset_group = "All datasets")

validation$hoek = validation$all_results %>%
  filter(dataset == 'hoek') %>% 
  group_by(method, cell_type) %>% 
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(dataset_group = "Hoek")

validation$hoek_all_cells = validation$all_results %>%
  filter(dataset == 'hoek') %>% 
  group_by(method) %>% 
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(cell_type = ALL_CELLS, dataset_group = "Hoek")

validation$racle_all_cells = validation$all_results %>%
  filter(dataset == 'racle') %>% 
  group_by(method) %>% 
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(cell_type = ALL_CELLS, dataset_group = "Racle")

validation$schelker_all_cells = validation$all_results %>%
  filter(dataset == 'schelker_ovarian') %>% 
  group_by(method) %>% 
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(cell_type = ALL_CELLS, dataset_group = "Schelker")

validation$all = bind_rows(validation$all_datasets, validation$all_datasets_all_cells,
                           validation$hoek, validation$hoek_all_cells,
                           validation$racle_all_cells,
                           validation$schelker_all_cells)
...
p_validation = validation$all %>%
  rename(column=cell_type, category=dataset_group) %>%
  plot_cor_table() + ggtitle("Validation") + geom_vline(xintercept = 1.5)
...
exclude_cell_types = "Dendritic cell"
tmp_cell_types = res_mixing_study$log_slope %>% filter(!cell_type %in% exclude_cell_types) %>% pull(cell_type) %>% unique() 
tmp_methods = config$abs_methods_final
tmp_matrix = crossing(tmp_cell_types, tmp_methods)
colnames(tmp_matrix) = c("cell_type", "method")
p_abs_deviation = res_mixing_study$log_slope %>%
  ...
  inner_join(method_names) %>%
   ...
   ggplot(aes(x=cell_type, y=ifelse(is.na(log_slope), 0, log_slope), alpha=macrophage_only)) +
      ...
      geom_text(aes(label=ifelse(is.na(log_slope), "n/a", NA))) +
      ...
      xlab(NULL) +
      ylab("log2(slope)") +
      coord_flip(y=c(-3.5, 3.5)) +
      theme_title(hjust=0)

print(p_abs_deviation)
```

```{r rmse_plot, fig.height=6, fig.width=3}
res_mixing_study$rmse %>%
  right_join(tmp_matrix) %>%
  inner_join(method_names) %>%
  ...
  ggplot(aes(x=column, y=cell_type)) +
    ...
    scale_fill_distiller(type="div", palette = "RdYlGn", direction=-1, values=c(0, .02, .1, 1), na.value="white") +
    scale_alpha_manual(values=c("yes"=.3, "no"=1.)) +
    theme_benchmark() +
    theme(axis.text.x.top=element_text(angle = 90, vjust = .5, hjust=0),
            legend.position="none",
