...
ggsave("../results/figures/detection_limit_fp.eps", width = WIDTH, height=130, units = "mm", fallback_resolution = 600, device = cairo_ps(fallback_resolution = 600))
...
p = tmp_barplot_data %>%
  ...
  scale_fill_manual(values=color_scales$immune_cells, guide=guide_legend(ncol=2)) +
...
```{r fig.width = 6.69, fig.height=3.93, echo=FALSE, message=FALSE, fig.cap="Migration chart figure for paper. "}
# combine legend and plot
plot_right = plot_grid(rmgenes_plot_cd4, rmgenes_plot_dc,
                       nrow=2, rel_heights = c(0.5, 0.5), align = "v", labels = c("b", "c"))

plot_left = plot_grid(migration_plot, NULL, migration_legend, NULL, nrow=4, rel_heights = c(.65, .03, .15, 0.03), align = 'v', labels=c("a", ""), axis='t')

plot_grid(plot_left, plot_right,
          nrow=1, rel_widths = c(.65, .5), align = "h", axis="t")

ggsave("../results/figures/spillover_migration_chart.tiff", width=WIDTH, height=90, units = "mm", dpi=600)
ggsave("../results/figures/spillover_migration_chart.jpg", width=WIDTH, height=90, units = "mm", dpi=600)
```
