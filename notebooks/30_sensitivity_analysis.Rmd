...
```{r sens-plot, fig.width=4, fig.height=4, echo=FALSE, fig.cap="Sensitivity of the methods per cell type. The bars indicate the minimal percentage of infiltration at which the method first reliably detects the cell type. Opaque bars indicate that the methods does not provide an estimate for the cell type."}
sensitivity_plot %>% 
  ...
  ggplot(aes(x=input_cell_type, y=method)) +
    ...
    geom_text(aes(label=min_frac_text), size=HEATMAP_FONT_SIZE) +
