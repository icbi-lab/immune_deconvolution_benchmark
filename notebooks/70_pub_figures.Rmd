...
method_names = tibble(
  ...
  method_name = factor(names(config$deconvolution_methods), levels=sort(names(config$deconvolution_methods), decreasing = TRUE))
) %>% arrange(method_name)
...
mixing =  res_mixing_study$correlations %>%
    mutate(asterisk = if_else(cell_type == "Macrophage/Monocyte" & method %in% c("mcp_counter", "epic", "timer"), "yes", "no")) %>%
...
p_abs_deviation = res_validation$slope %>%
  select(-n) %>%
  # remove neutrophils, as only three samples
  ...
  mutate(cell_type_name = sprintf("%s [n=%i]", cell_type, n)) %>%
   ...
   ggplot(aes(x=factor(cell_type_name, levels=unique(cell_type_name) %>% sort(decreasing = TRUE)),
                       y=ifelse(is.na(slope), 1, slope))) +
...
p_abs_rmse = res_validation$rmse %>%
  ...
  mutate(column="RMSE") %>%
  filter(cell_type != "Neutrophil") %>% 
  ggplot(aes(x=column, y=factor(cell_type, levels=unique(cell_type) %>% sort(decreasing = TRUE)))) +
