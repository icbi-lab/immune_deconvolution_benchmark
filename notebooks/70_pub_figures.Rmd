...
mixing = bind_rows(
  ...
  res_mixing_study$within_sample %>%
    filter(method %in% within_methods) %>% 
    mutate(method = factor(method, levels = method_names$method)) %>% 
    mutate(column="within samples", category="within"))

# Validation Benchmark

validation = bind_rows(
  res_validation$between_sample %>%
    filter(method %in% between_methods) %>%
    mutate(column = "between samples"),
  res_validation$within_sample %>%
    filter(method %in% within_methods) %>%
    mutate(column = "within samples")) %>%
  select(method, pearson, column)
```
...
```{r, fig.height=9, fig.width=7, echo=FALSE, message=FALSE}
# combine heatmaps
...
plot_heatmap = plot_grid(plots_left[[1]], p2_mixing, plots_right[[1]],
          align = "h", ncol = 3,
          axis="tb",
          rel_widths = c(.42, .8, .20), labels = c("A", "B", "C"))
