...
```{r, cache=TRUE}
bulk_sum = sapply(colnames(schelker_ovarian$bulk_mat), function(donor) {
...
expr_all = bind_rows(
  ...
  bulk_sum %>% as_tibble(rownames="gene_symbol") %>% mutate(source="mean")
) %>% 
...
expr_all %>%
  mutate(bulk=log2(bulk+1), mean=log2(mean+1)) %>%
  ggplot(aes(x=bulk, y=mean)) + geom_point() + stat_cor() + facet_wrap(~sample) + 
...
all_results_bulk = lapply(immunedeconv::deconvolution_methods, function(method) {
  deconvolute(schelker_ovarian$bulk_mat, method) %>%
