...
Noise is $\sim\mathsf{N}(0, \text{noise_level})$. 
...
if(!file.exists(noise_robustness_file)) {
  all_results = foreach(method = immunedeconv::deconvolution_methods,
                        .final = function(x) setNames(x, immunedeconv::deconvolution_methods)) %dopar% {
                          deconvolute(sample_noise_mat, method) 
                        }
...
all_results3 %>% 
  ...
  geom_point() + 
  stat_cor() + 
  facet_wrap(method~cell_type, scales = "free", ncol=length(show_cell_types))
```
...
### Correlations
```{r, cache=TRUE, echo=FALSE}
make_cor = function(no_noise, true_fraction) {
  test_res = cor.test(no_noise, true_fraction, method="pearson")
  data.frame(
