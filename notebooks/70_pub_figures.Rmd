...
noise_sc = res_methods_validity$all_results %>%
  ...
  mutate(column="stability single cell/bulk", category="Schelker")
...
validation$hoek_all_cells = validation$all_results %>%
  ...
  group_by(method) %>% 
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(cell_type = ALL_CELLS, category = "Hoek") %>%
  mutate(column = cell_type)
...
validation$racle_all_cells = validation$all_results %>%
  ...
  group_by(method) %>% 
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(cell_type = ALL_CELLS, category = "Racle") %>%
  mutate(column = cell_type)
...
validation$schelker_all_cells = validation$all_results %>%
  ...
  group_by(method) %>% 
  do(make_cor(.$estimate, .$true_fraction)) %>%
  mutate(cell_type = ALL_CELLS, category = "Schelker") %>%
  mutate(column = cell_type)
...
```{r sens-spec-plot, include=FALSE}
# Specificity heatmap
tmp_sens = res_sensitivity$sensitivity %>%
  ...
  na.omit() %>%
  mutate(category="Sensitivity", cell_type=input_cell_type) %>%
  rename(estimate=min_frac, estimate_text=min_frac_text)

tmp_spec = res_sensitivity$specificity %>% mutate(category="Specificity")
...
p_abs_rmse = res_mixing_study$rmse %>%
  right_join(tmp_matrix) %>%
  inner_join(method_names) %>%
