...
results_with_gold_standard = all_results_tidy %>% 
  inner_join(true_cell_count, by=c("cell_type", "sample"="donor")) 
```
...
### Plot of estimated and true values
```{r}
true_cell_count %>%
  mutate(method="TRUE") %>%
  select(method, cell_type, sample=donor, estimate=true_fraction) %>%
  bind_rows(all_results_tidy %>% filter(method %in% abs_methods)) %>% 
