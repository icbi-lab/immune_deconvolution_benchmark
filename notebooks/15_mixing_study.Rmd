...
The cancer fraction is $\sim\mathcal{N}(`r round(cancer_cell_param$estimate[1], 2)`, `r round(cancer_cell_param$estimate[2], 2)`)$.
...
all_results = foreach(method=config$deconvolution_methods,
  ...
  deconvolute(bulk_eset, method, column="gene_symbol", indications = timer_indications, scale_mrna=FALSE)
}
```
...
correlations %>%
  ggplot(aes(x=method, y=pearson, color=macrophage_only)) +
...
log_slope = results_with_gold_standard %>%
  filter(method %in% abs_methods) %>%
  group_by(macrophage_only, cell_type, method) %>%
  do(get_slope(.)) %>%
  mutate(log_slope=log2(slope), ci_lower=log2(ci_lower), ci_upper=log2(ci_upper))

res_mixing_study$log_slope = log_slope

cell_type_method_mat = crossing(config$abs_methods_mixing, show_cell_types)
colnames(cell_type_method_mat) = c("method", "cell_type")
```

```{r, fig.width=12, fig.height=4, echo=FALSE, warning=FALSE, message=FALSE, fig.cap="Comparison of absolute methods. The bars indicate the log2(slope). Values < 0 indicate an under-prediction of the cell type, values > 0 an over-prediction respectively. "}
log_slope %>%
  right_join(cell_type_method_mat) %>%
  # na -> 0 workaround, otherwise text label does not show
  ggplot(aes(x=cell_type, y=ifelse(is.na(log_slope), 0, log_slope), alpha=macrophage_only)) +
    geom_bar(aes(fill=method), stat="identity") +
    geom_text(aes(label=ifelse(is.na(log_slope), "n/a", NA)), angle=0) + 
    ...
    coord_flip() + 
    ylab("log2(slope) of linear fit")
```

```{r abs_rmse, fig.width=12, fig.height=4, echo=FALSE, fig.cap="Comparison of absolute methods. The values show the RMSE"}
rmse = results_with_gold_standard %>%
  filter(method %in% abs_methods) %>% 
  mutate(square_error = (estimate-true_fraction)^2) %>%
  group_by(method, cell_type, macrophage_only) %>%
...
rmse %>% 
 right_join(cell_type_method_mat) %>%
 ggplot(aes(x=cell_type, y=ifelse(is.na(rmse), 0, rmse), alpha=macrophage_only)) +
    geom_bar(aes(fill=method), stat="identity") +
