...
validation$all_datasets = validation$all_results %>%
  ...
  mutate(category = "All datasets", column=cell_type_name)
...
plot_validation = plot_grid(p2_validation_all_datasets, p2_validation_hoek, p2_validation_racle, p2_validation_schelker, 
          align = "h", nrow = 1,
          axis="tb",
          hjust = -.1,
          label_size = 12,
          rel_widths = c(.69, .45, .11, .11))

plot_grid(plot_mixing, plot_validation, align="h", ncol=1, axis="lr", labels = "AUTO", rel_heights = c(.7, .33))

ggsave("../results/figures/summary.pdf", height = 210, width = WIDTH, units = "mm")
ggsave("../results/figures/summary.png", height = 210, width = WIDTH, units = "mm", dpi=600)
```
...
data %>%
  ggplot(aes(x=frac_immune_cells, y=mean)) + 
    ...
    scale_color_manual(values=c("red", "blue"), guide=guide_legend("performance measure", override.aes = list(alpha=1)))
...
ggsave("../results/figures/spillover_migration_chart.pdf", width=WIDTH, height=195, units = "mm")
ggsave("../results/figures/spillover_migration_chart.png", width=WIDTH, height=195, units = "mm", dpi=600)
```
