...
make_cell_fraction_vector = function(input_cell_type, n) {
  if(input_cell_type == "T cell CD4+") {
    ...
    cell_n =  c(n_background_cells, round(n*(1-ratio)), round(n*ratio))
    names(cell_n) = c(names(n_background_cells), "T cell CD4+ (non-regulatory)", "T cell regulatory (Tregs)")
  } else {
  ...
  res = foreach(input_cell_type = show_cell_types,
    ...
    foreach(method = immunedeconv::deconvolution_methods,
      ...
      expr_mat = lapply(n_immune_cells, function(n) {
        cell_n = make_cell_fraction_vector(input_cell_type, n)
...
all_results = foreach(input_cell_type = show_cell_types, 
  ...
  foreach(method = immunedeconv::deconvolution_methods,
    ...
    n_cell_df = tibble(sample=colnames(tmp_res),
                       ...
                       n_background_cells = sum(n_background_cells)) 
  ...
  select(-sample) %>% 
  mutate(frac_immune_cells = as.factor(round(n_immune_cells/n_background_cells * 100, 2))) %>% 
...
results_inf_fallback = all_results %>%
