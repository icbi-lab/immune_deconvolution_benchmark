...
all_results_racle = lapply(immunedeconv::deconvolution_methods, function(method) {
  deconvolute(racle$expr_mat, method) %>%
...
all_results_hoeck = lapply(immunedeconv::deconvolution_methods, function(method) {
  deconvolute(hoeck$expr_mat, method) %>%
    mutate(method=method) %>%
...
```{r, cache=TRUE}
hoeck_ref2 = hoeck$ref %>% as_tibble(rownames="sample") %>% gather(cell_type, true_fraction, -sample)
...
racle_ref2 = racle$ref %>% rename(sample=`donor ID`) %>% gather(cell_type, true_fraction, -sample) %>% mutate(true_fraction=true_fraction/100)
...
schelker_ref2 = schelker_ovarian$ref
