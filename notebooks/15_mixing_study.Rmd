...
within_sample = results_with_gold_standard %>% 
  group_by(method, sample) %>% 
  do(make_cor(.$estimate, .$true_fraction))

average = within_sample %>% 
  group_by(method) %>% 
...
within_sample %>%
  ...
  mutate(fold=floor(sample_id/34)) %>% 
  mutate(pearson_text = as.character(round(pearson, 2))) %>%
  ggplot(aes(x=sample, y=method)) +
    ...
    facet_wrap(~fold, nrow=3, drop = TRUE, scales = "free_x") +
    theme(axis.text.x=element_text(angle = 90, vjust = .5, hjust=1))


res_mixing_study$within_sample = average
