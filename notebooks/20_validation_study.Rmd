...
Therefore, we normalize all scores per-celltype and re-do the correlations. 
```{r, fig.width=10, fig.height=16, echo=FALSE}
all_results_ref %>%
  filter(!(cell_type == "T cell" & dataset != "hoeck")) %>% 
  group_by(dataset, method, cell_type) %>% 
  mutate(estimate_norm=estimate/max(estimate),
         true_fraction_norm=true_fraction/max(true_fraction)) %>%
  ungroup() %>% 
  ggplot(aes(x=true_fraction_norm, y=estimate_norm)) +
