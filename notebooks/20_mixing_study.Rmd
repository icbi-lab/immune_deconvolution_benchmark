...
cell_fractions = lapply(is_melanoma, function(melanoma) {
  ...
  rnd = sample(0:100, length(cell_types$immune_cells), TRUE)
  tmp_fractions = (remaining_fraction * rnd) / sum(rnd)
  names(tmp_fractions) = cell_types$immune_cells
...
```{r, cache=TRUE}
all_results = sapply(immunedeconv::deconvolution_methods, function(method) {
...
all_results3 = lapply(names(all_results2), function(method) {
  map_result_to_celltypes(all_results2[[method]], single_cell_schelker$cell_types, method) %>%
