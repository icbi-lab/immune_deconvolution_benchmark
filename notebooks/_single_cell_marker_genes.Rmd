...
mat %>% 
  mutate(cell_type = ifelse(!cell_type %in% c("Dendritic cells", "B cells"), "other", cell_type)) %>% 
...
```{r, echo=FALSE, message=FALSE, fig.cap="Correlation of marker genes with increase of the amount of a certain cell type. Along x-axis: cell type used for simulation. Along y-axis: groups of marker genes. ", fig.height=12, fig.width=12}
...
dc_expression = foreach(i=1:10,
        .combine = bind_cols,
        .final = as.matrix) %do% {
          cell_n = make_cell_fraction_vector("Dendritic cells", k=500, background=NULL)
...
res_before = foreach(method=use_methods, 
                     .combine=bind_rows) %do% {
                       deconvolute(dc_expression, method=method) %>% 
...
res_after = foreach(method=use_methods, 
                     .combine=bind_rows) %do% {
                       deconvolute(dc_expression, method=method, rmgenes=c("TCL1A", "TCF4", "CD73", "SPIB", "BCL11A")) %>% 
                         map_result_to_celltypes(show_cell_types, method) %>% 
                         as_tibble(rownames = "cell_type") %>% 
...
```{r, fig.width=12, fig.cap="Predicted", echo=FALSE, message=FALSE, fig.cap="Predictions on 10 simulated DC samples before and after removal of the five genes. "}
res_all %>% 
  ggplot(aes(x=dataset, y=predicted_fraction, colour=dataset)) +
   ...
   facet_grid(method~cell_type, drop = TRUE) + 
