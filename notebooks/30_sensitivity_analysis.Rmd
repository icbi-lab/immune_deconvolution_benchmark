...
all_results2 = all_results %>%
  ...
  mutate(frac_immune_cells_numeric = as.numeric(as.character(frac_immune_cells))) %>% 
  ungroup()
```

## Predictions with increasing immune cell content
```{r, fig.height=18, fig.width=32, echo=FALSE, fig.cap="We add an increasing amount of each cell type independently to the 1,800 background cells. The figure shows the predictions for each cell type and method at increasing infiltration levels. "}
all_results2 %>%
  ggplot(aes(x=frac_immune_cells_numeric, y=mean, colour=cell_type)) +
  #   geom_line() +
