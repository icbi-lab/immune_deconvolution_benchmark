...
```{r hierarchy, fig.height=8, fig.width=8, echo=FALSE, fig.cap="Hierarchy of immune cell types used for mapping cell types between methods and datasets."}
...
tmp_env = new.env()
with(tmp_env, {
  load("../data/hoek/HoekPBMC_mixture.RData")
...
hoek$ref = tmp_env$RefData %>%
  ...
  inner_join(immunedeconv::cell_type_mapping %>% filter(method_dataset == "hoek"))
```
...
```{r racle_tab, message=FALSE, echo=FALSE, results="asis"}
...
```{r, fig.height=6, fig.width=10, echo=FALSE, fig.cap="Histogram of gene expression data of all datasets"}
par(mfrow=c(2,3))
...
```{r, fig.width=10, fig.height=6, echo=FALSE, fig.cap="Histogram of log-tranformed gene expression data of all datasets"}
par(mfrow=c(2,3))
