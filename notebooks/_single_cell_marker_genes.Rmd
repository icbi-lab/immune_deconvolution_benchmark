...
matrices = list(quantiseq=til10, epic=tref, cibersort=lm22)
...
res_after = foreach(method=use_methods, 
                     .combine=bind_rows) %do% {
                       deconvolute(dc_expression, method=method, indications=timer_indications, rmgenes=c("TCL1A", "TCF4", "CD37", "SPIB", "BCL11A", "IRF8")) %>% 
                         map_result_to_celltypes(show_cell_types, method) %>% 
                         as_tibble(rownames = "cell_type") %>% 
...
res_all %>% 
  ...
  ggplot(aes(x=dataset, y=predicted_fraction, colour=dataset)) +
    ...
    ylim(0, .5) + 
    ggtitle("spillover with and without signature genes removed.") + 
